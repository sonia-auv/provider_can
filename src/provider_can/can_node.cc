...
CanNode::CanNode(const ros::NodeHandlePtr &nh) ATLAS_NOEXCEPT
  ...
  if(devices_conf_.power_supply_en_)
    can_devices_vector_.push_back(
        std::make_shared<provider_can::PowerSupply>(can_ptr_, nh_, devices_conf_.psu_params_));
  ...
  if(devices_conf_.hydrophones_en_)
    can_devices_vector_.push_back(
        std::make_shared<provider_can::Hydrophones>(can_ptr_, nh_, devices_conf_.hydros_params_));
