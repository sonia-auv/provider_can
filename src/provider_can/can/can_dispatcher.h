...
namespace provider_can {
   ...
   * every message from every device is correctly processed. some messages such
   as
   ...
   * encountered
   * a fault. You must always verify the value of this return parameter to know
   if
   * your device is present or not. CanDispatcher will handle sending ID
   requests
   ...
   * and
   * getDeviceParams allows to read or modify these parameters. A list of
   possible
  ...
  class CanDispatcher : public atlas::Runnable {
   public:
    ...
    explicit CanDispatcher(uint32_t device_id, uint32_t unique_id,
    ...
    * \return SoniaDeviceStatus enum
    */
    SoniaDeviceStatus FetchComputerMessages(
    ...
    * known device.
    *
    * This function loops through all known device and compares the poll_rate
    * with
    * a timer value
    * to known if polling is required or not. use setPollRate() to specify a
    * poll
    * rate for a
    ...
    * devices
    * that answered. If a device did not answer to the ID request, no struct
    * will
    * be created
    ...
    */
    ...
    std::vector<CanMessage>
        rx_raw_buffer_;  // Buffer directly taken from KVaser
